...
namespace EFunTech.Sms.Portal.Controllers
{
    ...
    public class AccountController : System.Web.Mvc.Controller
    {
                ...
                case SignInStatus.Success:
                    {
                        ...
                        using (IUnitOfWork unitOfWork = new UnitOfWork(new ApplicationDbContext()))
                        {
                            ...
                            //ApplicationUser user = unitOfWork.Repository<ApplicationUser>().Get(p =>
                            ...
                            //else
                            //{
                            //    AuthenticationManager.SignOut(); // SignOut();
                            //    ModelState.AddModelError("", "登入嘗試失試，此帳號已被停用。");
                            //    return View(model);
                            //}
                        }
                    }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { Level = Int32.MaxValue - 1, UserName = model.Email, Email = model.Email };
